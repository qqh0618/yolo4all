    ...
    def run(self) -> None:
        ...
        while cap.isOpened():
            ...
            if success:
                ...
                # Visualize the results on the framez
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def result_util(self,index, img_array):
        print(f"get info {index}")
        if index==1:
            threading.Thread(target=self.thread_result_util, args=[self.label1, img_array]).start()
        elif index==2:
            threading.Thread(target=self.thread_result_util, args=[self.label2, img_array]).start()
        elif index==3:
            threading.Thread(target=self.thread_result_util, args=[self.label3, img_array]).start()
        elif index==4:
            threading.Thread(target=self.thread_result_util, args=[self.label4, img_array]).start()
