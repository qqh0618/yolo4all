...
class DetThread(QThread):
    ...
    def init_tracker(self):
        ...
        cfg_deep.merge_from_file(
            "E:/my_pro/YOLOv8-DeepSORT-Object-Tracking-main/YOLOv8-DeepSORT-Object-Tracking-main/deep_sort_pytorch/configs/deep_sort.yaml")

        init_deepsort = DeepSort(cfg_deep.DEEPSORT.REID_CKPT,
    ...
    def xyxy_to_xywh(self, *xyxy):
        ...
        w = bbox_w
        h = bbox_h
        return int(x_c), int(y_c), int(w), int(h)

    def draw_boxes(self, img, bbox, names, object_id, identities=None, offset=(0, 0), predicts=None):
        ...
        for key in list(data_deque):
            if key not in identities:
                data_deque.pop(key)
        for i, (box, single_pred) in enumerate(zip(bbox, predicts)):  # predicts的长度应该是9与bbox相同，single_pred长度是10，表示预测10步
            ...
            # code to find center of bottom edgexiankan
            center = (cx, cy)
            # print(center)
            org_xy = str(cx) + '_' + str(cy)
            # get ID of object
            ...
            for j in range(len(single_pred) - 2):
                px1, py1, px2, py2 = single_pred[j]
                p_center1 = (int((px2 + px1) / 2), int(py2 + (py1 - py2) * 0.2))

                px1, py1, px2, py2 = single_pred[j + 1]
                p_center2 = (int((px2 + px1) / 2), int(py2 + (py1 - py2) * 0.2))

                cv2.line(img, p_center1, p_center2, (0, 255, 0), 8)
