...
class DeepSort(object):
    ...
    def update(self, bbox_xywh, confidences, oids, ori_img):
        ...
        list_means, convs = self.tracker.update(detections)

        # output bbox identities
        outputs = []
        predicts = []
        for track, means in zip(self.tracker.tracks, list_means):
            ...
            # -------------------------------------
            # 预测数据转换
            xyxys = self.to_tlwh(means)  # 预测10步的框
            predicts.append(xyxys)
            # -------------------------------------
        if len(outputs) > 0:
            outputs = np.stack(outputs, axis=0)
        return outputs, predicts

    def to_tlwh(self, means):
        ...
        for mean in means:
            ...
            ret[2] *= ret[3]
            ret[:2] -= ret[2:] / 2
            x1, y1, x2, y2 = self._tlwh_to_xyxy(ret)
            rets.append([x1, y1, x2, y2])
        return np.array(rets, dtype=np.int)
    """
    TODO:
